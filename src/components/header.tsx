...
interface HeaderProps {
    onToggleStyleControl: () => void;
}

export function Header({ onToggleStyleControl }: HeaderProps) {
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20">
      <div className="flex items-center gap-4 rounded-full bg-card/80 backdrop-blur-sm p-2 pl-4 border border-border shadow-lg">
        ...
        <Button variant="ghost" size="icon" className="rounded-full" onClick={onToggleStyleControl}>
            <Settings />
        </Button>
      </div>
    </div>
